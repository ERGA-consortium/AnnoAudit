process FEATURECOUNTS {
    ...
    if (!params.rnaseq_single) {
        ...
        awk \'NR>1 {total++; if (\$NF>1) nonzero++} END {printf \"num_gene_supported\\t%d (%.2f%%)\\n\", nonzero, (nonzero/total)*100}' gene_counts.txt >> unsupported_genes.txt

        featureCounts -T ${task.cpus} -p -g exon_id -B -O -a ${ch_gtf} -o exon_counts.txt ${ch_genome_bam}
        awk \'NR>1 {total++; if (\$NF>1) nonzero++} END {printf \"num_exon_supported\\t%d (%.2f%%)\\n\", nonzero, (nonzero/total)*100}' exon_counts.txt >> unsupported_genes.txt
        """
    } else {
        """
